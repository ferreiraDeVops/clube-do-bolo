...
function applyImageAsset(image, source, sizes) {
  const asset = imageAssets[source];
  if (!asset) { image.src = source; return; }
...
function keyboardCarouselIndex(key, index, count) {
...
categories.forEach((category, categoryIndex) => {
  ...
  category.files.forEach((file, index) => {
    ...
    const source = `${category.originalFiles?.includes(file) ? '' : (category.basePath ?? 'optimized/')}${category.folder}/${file}`;
    figure.innerHTML = `<img alt="${category.title} clube do bolo ${index + 1}" loading="lazy" decoding="async"><figcaption>${caption}</figcaption>`;
  ...
  if (isShowcase) {
    ...
    const updateActiveDot = () => {
      ...
      positions.forEach((position, index) => {
        if (Math.abs(position - grid.scrollLeft) < Math.abs(positions[active] - grid.scrollLeft)) active = index;
      });
      if (active === previousActiveDot) return;
      const visibleStart = Math.max(0, Math.min(active - 1, positions.length - 3));
      const oldOffsets = new Map(Array.from(dots.children)
        .filter((dot) => !dot.hidden)
        .map((dot) => [dot, dot.offsetLeft]));
      Array.from(dots.children).forEach((dot, index) => {
        dot.hidden = index < visibleStart || index >= visibleStart + 3;
        dot.classList.toggle('active', index === active);
        dot.setAttribute('aria-current', String(index === active));
      });
      if (previousActiveDot >= 0 && active !== previousActiveDot &&
          !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        const direction = active > previousActiveDot ? 1 : -1;
        Array.from(dots.children).filter((dot) => !dot.hidden).forEach((dot) => {
          const entering = !oldOffsets.has(dot);
          const distance = entering ? direction * 48 : oldOffsets.get(dot) - dot.offsetLeft;
          const isActive = dot === dots.children[active];
          const startX = distance || direction * 12;
          dot.getAnimations().forEach((animation) => animation.cancel());
          dot.animate([
            { transform: `translateX(${startX}px) scale(${entering ? .45 : 1})`, opacity: entering ? 0 : 1, offset: 0 },
            { transform: `translateX(${-direction * 7}px) scale(${isActive ? 1.3 : 1.05})`, opacity: 1, offset: .55 },
            { transform: `translateX(${direction * 3}px) scale(${isActive ? .94 : 1})`, opacity: 1, offset: .8 },
            { transform: "translateX(0) scale(" + (isActive ? 1.5 : 1) + ")", opacity: 1, offset: 1 },
          ], { duration: 560, easing: 'ease-out', pseudoElement: '::before' });
        });
      }
      previousActiveDot = active;
      if (dots.contains(document.activeElement) && document.activeElement.hidden) {
        dots.children[active]?.focus({ preventScroll: true });
      }
    };
    const updateDots = () => {
      ...
      grid.querySelectorAll('.product-card').forEach((card) => {
        const position = Math.min(maxScroll, card.offsetLeft - first.offsetLeft);
        if (!positions.length || position - positions[positions.length - 1] > 2) positions.push(position);
      });
      if (dots.children.length !== positions.length) {
        ...
        positions.forEach((position, index) => {
          ...
          dot.type = 'button';
          dot.setAttribute('aria-label', `${category.title}: posição ${index + 1}`);
      ...
      dots.hidden = maxScroll <= 2;
      previousActiveDot = -1;
      updateActiveDot();
    };
    let dotFramePending = false;
    ...
    grid.addEventListener('scroll', scheduleDotUpdate, { passive: true });
    ...
    dots.addEventListener('keydown', (event) => {
      ...
      if (index < 0) return;
      const targetIndex = keyboardCarouselIndex(event.key, index, positions.length);
      if (targetIndex === null) return;
      event.preventDefault();
      // Update the visible dot window before moving keyboard focus.
      grid.scrollTo({ left: positions[targetIndex], behavior: 'instant' });
      updateActiveDot();
      dots.children[targetIndex]?.focus({ preventScroll: true });
    });
    updateDots();
  }
});
...
function showReview(index) {
  const previousIndex = currentReview;
  currentReview = index;
  reviewSlides.forEach((slide, slideIndex) => slide.classList.toggle('active', slideIndex === index));
  const visibleStart = Math.max(0, Math.min(index - 1, reviewButtons.length - 3));
  reviewButtons.forEach((button, buttonIndex) => {
    button.hidden = buttonIndex < visibleStart || buttonIndex >= visibleStart + 3;
    button.classList.toggle('active', buttonIndex === index);
    button.setAttribute('aria-current', String(buttonIndex === index));
    if (!button.hidden && previousIndex !== index && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      const direction = index === 0 && previousIndex === reviewButtons.length - 1 ? 1 : Math.sign(index - previousIndex);
      button.getAnimations().forEach((animation) => animation.cancel());
      button.animate([
        { transform: `translateX(${direction * 24}px) scale(.75)`, opacity: .4 },
        { transform: `translateX(${-direction * 7}px) scale(${buttonIndex === index ? 1.3 : 1.05})`, opacity: 1, offset: .55 },
        { transform: "translateX(0) scale(" + (buttonIndex === index ? 1.5 : 1) + ")", opacity: 1 },
      ], { duration: 560, easing: 'ease-out', pseudoElement: '::before' });
    }
  });
  if (Array.from(reviewButtons).some((button) => button === document.activeElement && button.hidden)) {
    reviewButtons[index].focus({ preventScroll: true });
  }
}
...
reviewButtons.forEach((button, index) => {
  button.addEventListener('keydown', (event) => {
    const targetIndex = keyboardCarouselIndex(event.key, index, reviewButtons.length);
    if (targetIndex === null) return;
    event.preventDefault();
    showReview(targetIndex);
    reviewButtons[targetIndex].focus({ preventScroll: true });
    startReviews();
  });
...
const decodedHeroImages = new Map();
if (heroCardImage) {
  setInterval(async () => {
    if (heroCardLoading || document.hidden || !heroVisible || reducedMotion.matches || !heroCardImage.getClientRects().length) return;
    heroCardLoading = true;
    const nextIndex = (heroCardIndex + 1) % heroCardSlides.length;
    const slide = heroCardSlides[nextIndex];
    try {
      if (!decodedHeroImages.has(slide.src)) {
        const nextImage = new Image();
        applyImageAsset(nextImage, slide.src, '(max-width: 768px) 70vw, 440px');
        await nextImage.decode();
        decodedHeroImages.set(slide.src, nextImage);
      }
      applyImageAsset(heroCardImage, slide.src, '(max-width: 768px) 70vw, 440px');
...
async function prepareHeroBackgrounds() {
  ...
  try {
    // Decode one background at a time without blocking the initial render.
    for (const source of ['optimized/Diversos/BoloFatiaPote1.JPG', 'Diversos/BoloFatiaPote2.png', 'Diversos/BoloFatiaPote3.png']) {
      ...
      await new Promise((resolve, reject) => {
        ...
        photo.src = window.matchMedia('(max-width: 768px)').matches ? imageAssets[source].small : imageAssets[source].src;
      });
...
// Pause decorative animations outside the viewport.
if ('IntersectionObserver' in window) {
  const animationObserver = new IntersectionObserver((entries) => {
    entries.forEach(({ target, isIntersecting }) => target.classList.toggle('offscreen', !isIntersecting));
  });
  document.querySelectorAll('.display-section, .quote-section, .site-footer').forEach((section) => animationObserver.observe(section));
}
